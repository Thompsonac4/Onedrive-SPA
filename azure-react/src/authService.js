...
class AuthService {
  ...
  /**
   * Starts interactive sign-in via full-page redirect (not popup).
   * User leaves this tab → Microsoft login → /auth.html → back to /.
   * This promise usually does not resolve in-page; the browser navigates away.
   */
  async login() {
  ...
  /**
   * Signs the user out via redirect and clears the local MSAL session.
   * Lands on postLogoutRedirectUri (auth.html → home in our setup).
   */
  async logout() {
    await this.initialize();
    this.#clearInteractionInProgress();
    const account = this.getAccount();
    debug("logoutRedirect for", account?.username);
  ...
  async getAccessToken(scopes = loginRequest.scopes) {
    ...
    try {
      ...
      // Expired session, missing consent, or Conditional Access — need a redirect.
