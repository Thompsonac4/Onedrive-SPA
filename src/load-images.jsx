...
export async function fetchPreviewUrl(driveId, itemId) {
    ...
    return data.getUrl + "&wdAr=1";
}

/**
 * Returns a short-lived, streamable download URL for a video (or any file).
 *
 * GET item metadata and use @microsoft.graph.downloadUrl — it already includes
 * auth, supports HTTP range requests (seeking), and does NOT require downloading
 * the whole file into a blob first.
 *
 * @param {string} driveId
 * @param {string} itemId
 * @returns {Promise<string>}
 */
export async function fetchVideoUrl(driveId, itemId) {
    const accessToken = await authService.getAccessToken();
    ...
    if (!downloadUrl) {
        throw new Error("No @microsoft.graph.downloadUrl on item");
    }
